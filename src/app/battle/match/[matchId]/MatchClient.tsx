...
export default function MatchClient({ matchId }: { matchId: string }) {
  ...
  async function nextProblem() {
    ...
    try {
      ...
      const j = await r.json();
      if (!r.ok) throw new Error(j?.error ?? 'Failed to serve next problem');
  ...
  async function submit() {
    ...
    try {
      ...
      if (j.correct) {
        setFeedback({ type: 'ok', msg: j.matchEnded ? 'Correct — you won the match!' : 'Correct!' });
        setAnswer('');
      } else {
        setFeedback({ type: 'bad', msg: j.message ?? 'Incorrect' });
      }
      await load();
    } catch (e: any) {
      setErr(e?.message ?? 'Failed');
          ...
          <div className="md:col-span-2 rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4">
            ...
            <div className="mt-4 rounded-xl border border-zinc-800 bg-zinc-950/30 p-4">
              {state?.currentProblem ? (
                <BlockMath math={state.currentProblem.latex} />
              ) : (
                <div className="text-sm text-zinc-400">No problem served yet. Host must click “Next problem”.</div>
              )}
            </div>

            <div className="mt-4 flex flex-col gap-2">
              <label className="text-xs text-zinc-400">Your answer (nerdamer-friendly expr for MVP)</label>
              <input
                ...
                disabled={!state?.currentProblem || finished}
              />
              ...
              {finished && (
                <div className="mt-2 text-sm text-zinc-300">
                  Match finished. Winner: <span className="font-mono">{state?.match?.winnerUserId}</span>
                </div>
              )}
            </div>
          </div>
